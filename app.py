...
def make_label_dict(label_headered_column_lists):
    ...
    for column_list in label_headered_column_lists:
        label_dict[column_list[0]] = [{'widget_type': 'Label', 'text': text, 'context': 'label_frame'} for text in column_list[1:]]
    return label_dict
...
def make_btn_run_cell_dlist(filename_list):
    ...
    for filename in filename_list:
        if '.csh' in filename:
            btn_run_cell_dlist.append({'widget_type': 'Button', 'text': 'run', 'context': 'button_frame'})
        else:
            btn_run_cell_dlist.append({'widget_type': 'Label', 'text': '-', 'context': 'button_frame'})
...
def make_btn_edit_cell_dlist(filename_list):
    ...
    for _ in range(len(filename_list)):
        btn_edit_cell_dlist.append({'widget_type': 'Button', 'text': 'edit', 'context': 'button_frame'})
